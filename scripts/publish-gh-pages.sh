...
# We don't copy charts to $INDEX_DIR anymore. We use the GitHub Release URL.
# The version is passed in the .releaserc.json environment, but we can extract it or assume standard tagging.

# Semantic release tag format: v${nextRelease.version} (default)
# URL: https://github.com/${GITHUB_REPOSITORY}/releases/download/v${VERSION}/${CHART_NAME}-${VERSION}.tgz
